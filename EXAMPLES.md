...
### Example 1: Ignoring Prior Context

**User Request:** "Continue the migration plan from last time"

**❌ What LLMs Do (Start From Scratch)**

```
I'll inspect the repository and come up with a migration plan.

[Proceeds without checking memory for prior decisions, constraints, or rejected approaches]
```

**Problems:**
- Re-discovers context the user already established
- Risks contradicting prior decisions
- Wastes time on already-resolved questions

**✅ What Should Happen (Check Memory Early)**

```
I'll check Zaxy/memory for the prior migration context first, then verify it against
the current repository state before making changes.

Memory says the previous decision was:
- Keep the old API until consumers move
- Migrate one module at a time
- Avoid changing storage format in this phase

I'll confirm the current code still matches those assumptions before proceeding.
```

### Example 2: Storing Speculation as Fact

**User Request:** "Why is this endpoint slow?"

**❌ What LLMs Do (Bad Memory Hygiene)**

```
Memory note:
The endpoint is slow because the database query lacks an index.
```

**Problems:**
- Records an unverified hypothesis as fact
- Can mislead future work
- Omits what was actually observed

**✅ What Should Happen (Factual Memory)**

```
Memory note:
Observed /search takes ~800ms in local profiling. Main uncertainty is whether
time is spent in the database query or result serialization. Next step is to run
query timing with representative data.
```

**Only store:** verified observations, durable decisions, explicit user preferences, and useful next steps. Keep secrets and guesses out of memory.
...
| Use Zaxy for Memory | Starts over or records guesses as facts | Check memory early; store only durable, factual context |
