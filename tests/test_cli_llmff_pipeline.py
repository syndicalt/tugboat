...
def test_audit_rejects_llmff_audit_with_unknown_instruction_refs(tmp_path: Path, capsys):
    ...
    fake_llmff = _write_fake_llmff(
        ...
        audit_report={
            ...
            "confidence": 0.91,
            "evidence_refs": ["ev_fake"],
            "instruction_refs": ["CODEX.md#does-not-exist"],
        },
    )
    policy_dir = repo / ".sidecar"
    policy_dir.mkdir()
...
def test_propose_rejects_drift_evidence_refs_not_declared_by_audit(
    ...
    fake_llmff = _write_fake_llmff(
        tmp_path / "fake-llmff",
        drift_clusters={"clusters": [{"cluster_id": "drift-1", "evidence_refs": ["ev_hallucinated"]}]},
    )
    policy_dir = repo / ".sidecar"
    policy_dir.mkdir()
    (policy_dir / "policy.yaml").write_text(
...
""".lstrip(),
    ...
    output = capsys.readouterr().out
    run_dir = sorted((repo / ".sidecar" / "runs").iterdir())[-1]
    assert "drift.raw.json evidence refs not declared by audit evidence: ev_hallucinated" in output
    assert (run_dir / "drift.raw.json").exists()
    assert not (run_dir / "candidate.json").exists()
    assert not (run_dir / "candidate.diff").exists()


def test_propose_rejects_optimizer_note_evidence_refs_not_declared_by_audit(
    tmp_path: Path,
    capsys,
):
    repo = tmp_path / "repo"
    ...
    fake_llmff = _write_fake_llmff(
        ...
        optimizer_notes={
            "notes": [
                {
                    "summary": "Use hallucinated evidence.",
                    "evidence_refs": ["ev_hallucinated"],
                }
            ]
        },
...
def test_eval_preserves_pending_eval_definition_paths_when_regating_candidate(
    ...
    fake_llmff = _write_fake_llmff(
        ...
        audit_report={
            ...
            "confidence": 0.91,
            "evidence_refs": ["ev_fake"],
            "instruction_refs": ["tests/fixtures/evals/regression.json#bytes-0-24"],
        },
        bounded_edit_metadata=[
            {
                "operator": "replace",
...
def test_eval_rejects_accept_recommendation_without_explicit_governance_result(
    tmp_path: Path,
    capsys,
):
    repo = tmp_path / "repo"
...
""".lstrip(),
    ...
    run_dir = sorted((repo / ".sidecar" / "runs").iterdir())[-1]
    report = json.loads((run_dir / "eval-report.json").read_text(encoding="utf-8"))
    assert "llmff eval_report cannot accept without governance result" in output
    assert report["passed"] is False
