...
`tugboat_auto_update_status` reports the current auto-apply enablement state, kill-switch state, paused lanes, daemon queue summary, and lane counters derived from append-only audit events. Pause-control decisions are exposed as paused lane activity instead of rejected activity, which keeps MCP consumers from treating operator pauses as policy failures.
