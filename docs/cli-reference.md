...
- `index` parses configured instruction files and stores document/chunk metadata. Add `--check` for dry-run validation without writing index rows. Instruction discovery honors `instruction_files[].scope_root` for monorepos, then applies policy-owned `index.max_instruction_files` and blocks before parsing or DB writes when the budget is exceeded.
- `status` writes `.sidecar/status-report.json` and prints mode, auto-apply state, latest run, latest `llmff` job, pending candidates, retention candidates, manifest policy, and daemon queue health.
...
`tugboat eval --repo . --candidate latest --suite all` writes `.sidecar/runs/<run-id>/eval-report.json`. Offline eval metrics include `instruction_token_delta`, `duplicate_rule_token_delta`, `instruction_token_growth_reason`, and `instruction_token_growth_acceptable` so reviewers can tell whether token growth was paired with held-out improvement and governance safety. Duplicate rule growth without held-out improvement is reported as `duplicate_token_growth_without_held_out_improvement`. For `SKILL.md` preview rewrites, the report includes `skill_report` with deterministic checks for trigger preservation, executability, ambiguity, overfit risk, token footprint, required sections, explicit non-goals, examples or fixtures when present, forbidden sections, safety weakening, and held-out skill behavior. Skill reports include `metrics.held_out_behavior_score`, while the top-level eval metrics include `skill_held_out_behavior_cases` and `skill_held_out_behavior_failures`. A failing `skill_report` forces `passed: false` and `recommendation: reject`.
...
tugboat ops observability --repo . --event-log-output .sidecar/ops/observability/events.jsonl
...
Operations commands write reviewable artifacts under `.sidecar/ops`. Retention and redaction scans honor policy-owned `retention.max_scan_files` and block before deletion or export when `.sidecar/runs` exceeds the budget. `ops observability --metrics-output` writes a local Prometheus text-format metrics export derived from the same summary artifact, with bounded labels for failure kinds, daemon queue states, and auto-apply lanes. `ops observability --event-log-output` writes deterministic JSONL operational events for summary, daemon queue, and auto-apply lane state without raw trace text, candidate rationales, or evidence payloads. `ops release-manifest` verifies retained evidence for doctor, index check, harness check, CI report, pytest coverage at or above 90%, wheel build, twine check, installed-wheel smoke, security review, and the current HEAD commit before writing `.sidecar/ops/release-artifact-manifest.json`. Destructive operations require explicit apply or execute flags and are blocked by the read-only kill switch where applicable. `ops migrate` blocks if `.sidecar/version.json` or legacy `.sidecar/VERSION` is newer than this Tugboat binary supports; unmarked 0.x sidecars migrate as legacy schema v1, and `policy.yaml version` is preserved as policy metadata. Upgrade Tugboat before using or migrating a future-version sidecar. See `docs/migration-v1.md` for upgrade procedure and `docs/compatibility-policy.md` for the v1 CLI and artifact compatibility policy.

## Exit Codes

Exit code `0` means the requested command completed its documented success path. For read commands this means output was produced. For write-capable commands this means the required artifact, VCS action, or explicit dry-run report was produced.

A nonzero exit means the command was blocked, rejected, failed validation, or could not publish required evidence. Safety gates use nonzero exits instead of partial mutation, including invalid policy, missing or malformed traces, failed eval or governance gates, dirty worktrees, stale base hashes, read-only kill switch blocks, missing rollback metadata, and release-manifest evidence failures.

`auto-apply --preflight` exits `0` when the preflight report is written, even when the report says the candidate is ineligible. `auto-apply --shadow` exits `0` when shadow evidence is written without mutation. Confirmed auto-apply exits nonzero unless all policy, eval, VCS, rollback, shadow, and incident gates pass.

`ops release-manifest` exits nonzero when required release evidence is missing, stale, below threshold, or fails security review. It writes `.sidecar/ops/release-artifact-manifest.json` only after validating the wheel identity, coverage evidence, CI/harness evidence, build and twine evidence, installed-wheel smoke, security review, and commit identity.
