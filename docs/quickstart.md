...
Review `.sidecar/runs/<run-id>/candidate.diff`, `eval-report.json`, `optimization-summary.json`, and `report.md`. When the candidate rewrites `SKILL.md`, inspect `eval-report.json` `skill_report` before applying; failing trigger, executability, ambiguity, overfit, held-out skill behavior, section, token-footprint, or safety checks should stay in review.
