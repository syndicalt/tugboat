...
For `skill_improvement`, `eval-report.json.skill_report.passed` must be true and the report must include held-out skill behavior evidence: `metrics.skill_held_out_behavior_cases > 0`, `metrics.skill_held_out_behavior_failures == 0`, and `skill_report.metrics.held_out_behavior_score >= 1.0`. Missing or failing held-out skill behavior evidence blocks with `skill_held_out_behavior_failed`. Skill rewrites that remove explicit non-goals or examples or fixtures from an existing `SKILL.md` fail the skill report and remain review-only.
